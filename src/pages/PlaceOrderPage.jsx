...
const PlaceOrderPage = () => {
  ...
  // Set default payment method to COD if none is selected
  useEffect(() => {
    if (!shippingAddress) {
      navigate('/shipping');
    }
    if (!paymentMethod) {
      toast.info('Default payment method set to Cash on Delivery (COD)');
    }
  ...
  const placeOrderHandler = async () => {
    try {
      const res = await createOrder({
        cartItems,
        shippingAddress,
        paymentMethod: paymentMethod || 'Cash on Delivery', // Set COD if paymentMethod is not set
        itemsPrice,
        taxPrice,
        shippingPrice,
        totalPrice,
        isPaid: false, // Indicate that payment is not done
      }).unwrap();
        ...
        <Col md={8}>
          <ListGroup variant="flush">
            ...
            <ListGroup.Item>
              ...
              <strong>Method:</strong> {paymentMethod || 'Cash on Delivery'}
            </ListGroup.Item>
            <ListGroup.Item>
              <h2>Order Items </h2>
              <ListGroup variant="flush">
